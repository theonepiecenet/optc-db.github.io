    ...
    return {
        ...
        template: '<div id="hp"><div id="hpSlider"></div>' + 
            '<div id="hp-rcv"><table><tbody>' +
                ...
                '<tr ng-show="numbers.healPerTurn"><td>heal/turn</td><td>{{ numbers.healPerTurn | number }}</td></tr>' +
                '<tr title="Minimum pirate level: {{ numbers.cost.level }}"><td>cost</td><td>{{ numbers.cost.cost | number }}</td></tr>' +
            '</tbody></table></div>',
        link: function(scope, element, attrs) {
